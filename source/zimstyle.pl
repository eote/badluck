...
use vars qw/%Action $Message @MyRules %RuleOrder $OpenPageName %Action $q $ImageExtensions $AllNetworkFiles $ModuleDir/;
use File::Spec;
    ...
    if($cond4) {
      ...
      #  $result = '<span>' . $exp . '</span>';
       ...
       #     $result .='<span>' . $_ . '</span>';
            ...
            my $text = $page_name;$text =~ s/(?:\/+$|^.*\/+)//;
            if( -d $_ || (/\.txt$/ && $page_name =~ s/\.txt$//)) {
                $page_name =~ s/\//:/g;
                $page_name =~ s/(?:^\:+|\:+$)//;
